						...
						<table id="example1" class="table table-bordered table-striped dataTable no-footer" role="grid" aria-describedby="example1">
							...
							<tbody>
								...
								if(count($lists) > 0){
									...
									?>
										<tr>
											<td><?php echo ucfirst($list['size_name']);?></td>
                                             <td>
											  <?php echo ucfirst($list['set_order']);?>
											 </td>
											<td>
											...
											<td>
											   ...
											   <a href="<?php echo $BASE_URL.$class_name.$sub_page_delete_url?>/<?php echo $list['id'];?>" style="color:#d71b23;padding: 5px;" title="delete"  onclick="return confirm('Are you sure you want to delete this size?');">
