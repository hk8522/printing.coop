    ...
    <tr>
      ...
      <td style="width: 50%;">
        <div style="text-align: right;">
          ...
          <h3 style="margin: 0px; font-size: 18px; font-weight: 600; color: #000;">Réf. : F-00<?=$orderData['id']?></h3>
          <p style="margin: 15px 0px 0px 0px; font-size: 18px; font-weight: 400; color: #000;">Réf. ordre:
            <?=ORDER_ID_PREFIX_FRENCH . $orderData['id']?> (GAL inc.) /
      ...
      <?php foreach ($OrderItemData as $rowid => $item) {
        ...
        <tr style="background-color: #fff; border-bottom: 1px dashed #ccc !important;">
          ...
          <?=ucfirst($item['name_french'] ?? $item['name'])?><br>
            ...
            <?php if ($votre_text) {?>
              <span><strong>Votre TEXTE - Votre TEXTE: <?=$votre_text?></strong></span><br>
            <?php }?>
          </td>
