...
Coating";
    ...
    public function DeleteSizeOptions($id = null, $type)
    {
        if (!empty($id)) {
            ...
            $this->load->model('Product_Model');
            if ($this->Product_Model->deleteFromTable($type, $id)) {
                $this->session->set_flashdata('message_success', $page_title . ' Successfully.');
            } else {
                $this->session->set_flashdata('message_error', $page_title . ' Unsuccessfully.');
            }
    ...
    public function UpdatedQtyTable()
    {
        set_time_limit(0);
        echo 'Cron Start...';
        $this->load->model('Product_Model');
    ...
    public function UpdatedTableData()
    {
        set_time_limit(0);
        echo 'Cron Start...';
        $this->db->select(array('*'));
    ...
    public function AutoExcelGen($product_id)
    {
        $this->load->model('Product_Model');
    ...
    public function AutoExcelGenCurrent($product_id)
    {
        $this->load->model('Product_Model');
        $product = $this->Product_Model->getProductDataById($product_id);
    ...
    public function uploadAttributes($product_id)
    {
        $this->load->model('Product_Model');
    ...
    public function uploadFullPriceList($product_id)
    {
        $this->load->model('Product_Model');
    ...
    public function Provider($provider = null)
    {
        ...
        $this->data['providers'] = $this->Provider_Model->getProviders();
        $this->data['provider'] = $provider;
        $this->render($this->class_name . 'providers');
    }
     ...
     * POST
     */
    public function ProviderProducts($provider)
    {
    ...
    public function ProviderProductUnbind($id)
    {
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            $this->Provider_Model->unbindProduct($id);
    ...
    public function Attributes()
    {
        ...
        $take = $pageSize;
        $skip = $pageSize * ($page - 1);
        $this->Product_Model->getAttributes($q, $take, $skip, $data, $total);
